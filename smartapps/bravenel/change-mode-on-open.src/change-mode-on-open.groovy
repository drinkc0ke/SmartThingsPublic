 ...
 */
definition(
    name: "Change mode on open",
    namespace: "bravenel",
    author: "Bruce Ravenel",
    description: "Change mode on open with restriction options",
    category: "My Apps",
...
def settings() {
	dynamicPage(name: "settings", title: "Change mode upon open", uninstall: true, install: true) {
		section {
			input "door", "capability.contactSensor", title: "Contact sensor", multiple: false, required: true
...
def initialize() {
	subscribe(door, "contact.open", openHandler)
}

def openHandler(evt) {
	if(allOk) location.setMode(newMode)
}
